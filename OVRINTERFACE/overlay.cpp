...
OverlayInterface::OverlayInterface()
    : m_strVRDriver("No Driver"), m_strVRDisplay("No Display"), m_eLastHmdError(vr::VRInitError_None), m_eCompositorError(vr::VRInitError_None), m_eOverlayError(vr::VRInitError_None), m_ulOverlayHandle(vr::k_ulOverlayHandleInvalid), m_pWidget(NULL)
{
}
...
vr::VROverlayHandle_t OverlayInterface::CreateInteractiveOverlay(const std::string &overlayName)
{
    ...
    if (bSuccess)
    {
        ...
        transformMatrix.m[0][1] = 0.0f;
        transformMatrix.m[0][2] = 0.0f;
        transformMatrix.m[0][3] = 0.0f;
        transformMatrix.m[1][0] = 0.0f;
        transformMatrix.m[1][1] = 1.0f;
        transformMatrix.m[1][2] = 0.0f;
        transformMatrix.m[1][3] = 0.0f;
        transformMatrix.m[2][0] = 0.0f;
        transformMatrix.m[2][1] = 0.0f;
...
vr::VROverlayHandle_t OverlayInterface::CreateAlignmentOverlay(bool mode)
{
    ...
    if (bSuccess)
    {
        ...
        transformMatrix.m[0][3] = 0.0f;
        transformMatrix.m[1][0] = 0.0f;
        transformMatrix.m[1][1] = 0.0f;
        transformMatrix.m[1][2] = 0.0f;
        transformMatrix.m[1][3] = 0.0f;
        transformMatrix.m[2][0] = 0.0f;
        transformMatrix.m[2][1] = -1.0f;
        transformMatrix.m[2][2] = 1.0f;
        transformMatrix.m[2][3] = 0.0f;
        vr::VROverlay()->SetOverlayTransformAbsolute(m_ulOverlayHandle, vr::TrackingUniverseStanding, &transformMatrix);
        vr::VROverlay()->ShowOverlay(m_ulOverlayHandle);
...
void LogHMDAndControllersPositions(vr::IVRSystem *pVRSystem)
{
    ...
    for (vr::TrackedDeviceIndex_t unDevice = 0; unDevice < vr::k_unMaxTrackedDeviceCount; ++unDevice)
    {
        ...
        if (!pVRSystem->IsTrackedDeviceConnected(unDevice))
            continue;
...
void OverlayInterface::SetOverlayPosition2(vr::VROverlayHandle_t overlayHandle, vr::HmdMatrix34_t overlayPosition)
{
    // Use HMD-relative positioning
    vr::TrackedDeviceIndex_t hmdIndex = vr::k_unTrackedDeviceIndex_Hmd;

    // Ensure the transformation is relative to the HMD
    // You might need to adjust this transformation based on your coordinate system
    vr::HmdMatrix34_t relativeTransform = overlayPosition;

    // Example: Move the overlay 1 meter in front of the HMD
    relativeTransform.m[2][3] = -0.1f; // Negative Z is forward in SteamVR

    vr::VROverlayError error = vr::VROverlay()->SetOverlayTransformTrackedDeviceRelative(overlayHandle, hmdIndex, &relativeTransform);
    if (error != vr::VROverlayError_None)
    {
        std::cerr << "Error setting overlay transform: " << vr::VROverlay()->GetOverlayErrorNameFromEnum(error) << std::endl;
    }
}
...
void OverlayInterface::HandleVRInput(vr::IVRSystem *pVRSystem, VROverlayHandle_t pOverlayToCheckFor)
{
    ...
    if ((!leftControllerButtonState.isPressed) && (!rightControllerButtonState.isPressed))
    {
